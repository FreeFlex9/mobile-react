...
export function EmpresaDashboard() {
  ...
  const load = useCallback(async (showSpinner = true) => {
    ...
    try {
      const res = await api.dashboardEmpresa();
      setData(res);
    } catch (err) {
  ...
  async function handleProposta(proposalId: number, aceitar: boolean) {
    setActionLoadingId(proposalId);
    try {
      if (aceitar) await api.aceitarPropostaEmpresa(proposalId);
      else await api.rejeitarPropostaEmpresa(proposalId);
      await load(false);
    } catch (err) {
      const e = err as ApiError;
  ...
  return (
    ...
    >
      <Text style={styles.greeting}>Olá, {data.company.nome}</Text>
      <StatusBanner status={data.company.status_aprovacao} />

      <View style={styles.statsGrid}>
        <StatCard label="Demandas abertas" value={data.stats.open_demands} />
        <StatCard label="Propostas pendentes" value={data.stats.pending_proposals} />
        <StatCard label="Agendamentos" value={data.stats.scheduled} />
        <StatCard label="Concluídas" value={data.stats.completed} />
      </View>

      <Text style={styles.sectionTitle}>Propostas pendentes</Text>
      {data.propostas_pendentes.length === 0 ? (
        <Text style={styles.emptyText}>Nenhuma proposta aguardando resposta.</Text>
      ) : (
        data.propostas_pendentes.map((p) => (
          <View key={p.id} style={styles.itemCard}>
            <Text style={styles.itemTitle}>{p.demanda_titulo}</Text>
            <Text style={styles.itemSubtitle}>Prestador: {p.prestador}</Text>
            {p.mensagem ? <Text style={styles.itemMeta}>“{p.mensagem}”</Text> : null}
            <View style={styles.actionsRow}>
              <TouchableOpacity
                style={[styles.actionBtn, styles.actionBtnOutline]}
                onPress={() => handleProposta(p.id, false)}
              ...
              <TouchableOpacity
                ...
                onPress={() => handleProposta(p.id, true)}
                disabled={actionLoadingId === p.id}
              >
      ...
      {data.proximos_agendamentos.length === 0 ? (
        ...
        data.proximos_agendamentos.map((a) => (
          <View key={a.id} style={styles.itemCard}>
            ...
            <Text style={styles.itemSubtitle}>{a.prestador} · {a.prestador_telefone}</Text>
      ...
      {data.demandas_recentes.length === 0 ? (
        <Text style={styles.emptyText}>Você ainda não criou demandas.</Text>
      ) : (
        data.demandas_recentes.map((d) => (
          <View key={d.id} style={styles.itemCard}>
            ...
            <Text style={styles.itemSubtitle}>{d.servico} · {d.vagas_confirmadas}/{d.vagas_necessarias} vagas</Text>
            <Text style={styles.itemMeta}>
              {formatDate(d.data)} · {formatTime(d.hora_inicio)}–{formatTime(d.hora_fim)} · {d.propostas_count} proposta(s)
            </Text>
          </View>
        ))
      )}
...
const STATUS_LABELS: Record<string, string> = {
  open: 'Aberta',
  partially_scheduled: 'Parcialmente preenchida',
